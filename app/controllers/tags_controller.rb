class TagsController < ApplicationController
  ...
  def show
    ...
    @theme_tagged_with = if user_signed_in?
                           @theme_tagged_with.select { |theme| theme.post_status == 2 or theme.user == current_user }
                         else
                           @theme_tagged_with.select { |theme| theme.post_status == 2 }
                         end
